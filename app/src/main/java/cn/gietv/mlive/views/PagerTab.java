...
import cn.gietv.mlive.utils.UIUtils;

public class PagerTab extends ViewGroup {
	String TAG = PagerTab.class.getName();
	...
	private int mDividerColor = 0xFFd9dfdc;//分割线颜色
	...
	private int mIndicatorColor = 0xFF4fc396;//指示器颜色
	...
	public void setTextColor(int colorId){
		mTabTextColorResId = colorId;
	}
	//设置背景的ID
	public void setTextBackgroundDrawable(int drawableId){
		mTabBackgroundResId = drawableId;
	}
	/** 初始化一些常量 */
	private void init() {
		//把一个值从dip转换成px
		mIndicatorHeight = DensityUtil.dip2px(mContext , mIndicatorHeight);
		mDividerPadding = DensityUtil.dip2px(mContext , mDividerPadding);
		mTabPadding = DensityUtil.dip2px(mContext , mTabPadding);
		mDividerWidth = DensityUtil.dip2px(mContext , mDividerWidth);
		mTabTextSize = DensityUtil.dip2px(mContext , mTabTextSize);
		//创建一个scroller
...
//		tab.setTypeface(Typeface.defaultFromStyle(Typeface.BOLD));
	...
	private void addTab(final int position, View tab) {
		...
		Log.e(TAG,"position = " + position + "    View" + ((TextView)tab).getText());
