...
public class CategoryAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder>{
    ...
    public CategoryAdapter(Context context, List<Object> objects,int positionFlag) {
    ...
    private void handleVideo(final ProgramBean.ProgramEntity bean, View convertView) {
        ...
        if(bean.name != null && bean.name.length() > 50)
            bean.name = bean.name.substring(0,50)  + "……";
        name.setText(bean.name);
        convertView.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
    ...
    public interface AdapterListener{
        void changeData(String tag, int positionFlag);
    }
    public void setAdapterListener(AdapterListener listener){
        this.adapterListener = listener;
    }
    public void setShowPopuWindowListener(ShowPopuWindowListener listener){
        this.showPopuWindowListener = listener;
    }
    public interface  ShowPopuWindowListener{
        void showPopuWindow(int height);
    }
