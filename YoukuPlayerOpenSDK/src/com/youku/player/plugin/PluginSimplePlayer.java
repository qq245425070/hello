...
public class PluginSimplePlayer extends PluginOverlay implements DetailMessage {
	...
	public ImageView fullSmallBtn;// 全屏按钮
     ...
     * 控制弹幕输入框显示和隐藏的图片
     * */
	public ImageView mDanmuImage;
	/**
	 * 弹幕输入框
	 * */
	public LinearLayout mDanmuParent;
	public EditText mDanmuContent;
	/*
    * 举报的图片
    * */
	...
	private void getMyView(View containerView){
		mDanmuImage = (ImageView) containerView.findViewById(R.id.danmu_image);

		mSendImage = (ImageView) containerView.findViewById(R.id.send_button);
		mDanmuSwitch = (ImageView) containerView.findViewById(R.id.danmu_switch);
		mDanmuParent = (LinearLayout) containerView.findViewById(R.id.danmu_input);
		mDanmuContent = (EditText) containerView.findViewById(R.id.danmu_text);
		mCancelText = (TextView) containerView.findViewById(R.id.cancel);
...
//		});
	...
	private void initPlayLayout() {
		...
		controlLayout = (LinearLayout) containerView
				.findViewById(R.id.control_parent);
	...
	public void setShowing(boolean isSmallScreen){
		this.mShowing = isSmallScreen;
	}
	private void hideControl() {
		if (null != controlLayout) {
			controlLayout.setVisibility(View.GONE);
			if(!mShowing){
				if(null != mDanmuParent)
	...
	public interface VideoPlayErrorListener{
		void onPlayErrorListener();
	}
	private VideoPlayErrorListener mListener;
	public void setVideoPlayErrorListener(VideoPlayErrorListener listener){
	...
	private void updateView() {
		changePlayPause();
		if (mMediaPlayerDelegate.isFullScreen) {
			LinearLayout.LayoutParams titleLayoutParams = (LinearLayout.LayoutParams) titleLayout
					.getLayoutParams();
...
//			titleBack.setImageResource(R.drawable.full_icon_back);// 返回按钮图片
			titleBack.setImageResource(R.mipmap.vertical_icon_back);
			int backPadPx = Util.dip2px(10);
...
//			fullSmallParams.rightMargin = Util.dip2px(20);
			...
			LinearLayout.LayoutParams titleLayoutParams = (LinearLayout.LayoutParams) titleLayout
					.getLayoutParams();
			titleLayoutParams.height = Util.dip2px(40);
			titleLayout.setLayoutParams(titleLayoutParams);
			titleLayout.setBackgroundColor(Color.TRANSPARENT);
